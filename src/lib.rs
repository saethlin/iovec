...
/// Attempts to convert slices longer than this value will result in a panic.
#[cfg(not(windows))]
pub const MAX_LENGTH: usize = std::usize::MAX;
...
impl IoVec {
    pub fn from_bytes(slice: &[u8]) -> Option<&IoVec> {
        ...
        assert!(slice.len() <= MAX_LENGTH);
        // SAFETY: IoVec has the same layout as [u8] because it is transparent
        Some(unsafe { &*(slice as *const [u8] as *const IoVec) })
    }
    ...
    pub fn from_bytes_mut(slice: &mut [u8]) -> Option<&mut IoVec> {
        ...
        assert!(slice.len() <= MAX_LENGTH);
        // SAFETY: IoVec has the same layout as [u8] because it is transparent
        Some(unsafe { &mut *(slice as *mut [u8] as *mut IoVec) })
    }
...
impl<'a> From<&'a [u8]> for &'a IoVec {
    fn from(bytes: &'a [u8]) -> &'a IoVec {
        IoVec::from_bytes(bytes).expect(
            "this crate accidentally accepted 0-sized slices \
...
impl<'a> From<&'a mut [u8]> for &'a mut IoVec {
    fn from(bytes: &'a mut [u8]) -> &'a mut IoVec {
        IoVec::from_bytes_mut(bytes).expect(
            "this crate accidentally accepted 0-sized slices \
                     originally but this was since discovered as a soundness \
...
impl<'a> Default for &'a IoVec {
    fn default() -> Self {
        panic!(
            "this implementation was accidentally provided but is \
